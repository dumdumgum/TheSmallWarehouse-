...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[CommonTableViewCell class] forCellReuseIdentifier:@"xiaoshou"];
    self.tableView.rowHeight = 60;
    self.sellArray = @[@"收银",@"销售单"];
    self.imgArray = @[@"shouyin",@"xiaoshoudan"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CommonTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"xiaoshou"];
    
    cell.nameLabel.text = self.sellArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        ShouYinViewController *shouyinVC = [[ShouYinViewController alloc] init];
